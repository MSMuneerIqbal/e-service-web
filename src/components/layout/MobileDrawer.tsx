 ...
 * than cosmetic. The header carries `backdrop-blur-sm`, and backdrop-filter
 * makes an element a containing block for position:fixed descendants. Rendered
 * in place, `fixed inset-0` resolved against the 72px header instead of the
 * viewport: the panel was 72px tall and every menu item laid out below its own
 * container, so the menu looked empty. Portalling escapes that containing
 * block. Do not move this back inside the header.
 */
export function MobileDrawer({
  ...
  return (
    <>
      ...
      {/*
        No mount guard needed: `open` starts false and can only become true
        from a click handler, which never runs during server rendering. By the
        time this branch is taken, document.body exists.
      */}
